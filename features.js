...
function setupAI(){
  document.getElementById('aiSubtaskBtn').addEventListener('click',()=>{
    ...
    if(subs){
      ...
      res.innerHTML='<p>No specific suggestions found. Try: Dynamic Programming, OS, DBMS, Graphs, Trees, etc.</p>';
    }
  });
  ...
  document.getElementById('aiScheduleBtn').addEventListener('click',()=>{
    ...
    const blocks=[
      {cat:'Exercise',pct:.10},{cat:'DSA',pct:.25},{cat:'Core Subjects',pct:.15},{cat:'Design',pct:.15},
      {cat:'Communication',pct:.10},{cat:'Projects',pct:.15},{cat:'Aptitude',pct:.10}
    ];
  ...
  document.getElementById('aiGeminiBtn').addEventListener('click',async()=>{
    ...
    const systemPrompt=`You are a study planner assistant. The user wants to plan their CS preparation schedule.
Categories available: Exercise, DSA, Core Subjects, Design, Communication, Projects, Aptitude.
Scope: ${scope}. Hours available per day: ${hours}. Today's date: ${today}.
Return ONLY a JSON array of task objects. Each task must have: title, category (from above list), startTime (HH:MM 24h format), endTime (HH:MM 24h format), priority (high/medium/low), date (YYYY-MM-DD format).
    ...
    try{
      ...
      document.getElementById('addAiPlanBtn').addEventListener('click',()=>{
        aiTasks.forEach(t=>{
          ...
          tasks.push({
            id:uid(),title:t.title,category:t.category||'DSA',date:t.date||today,
  ...
  for(let i=0;i<7;i++){
    ...
    if(dt.length){
      dt.sort((a,b)=>(a.startTime||'').localeCompare(b.startTime||'')).forEach(t=>{
        ...
        html+=`<div class="cal-timeline-item"><span class="cal-timeline-time">${time}</span>
...
function renderDaily(){
  ...
  dt.forEach(t=>{
    ...
    html+=`<div class="tl-block${isActive?' tl-active':''}${isDone?' tl-done':''}" style="top:${top}px;height:${height}px;--cat-color:${color}" data-id="${t.id}">
      ...
      <div class="tl-block-meta">${t.startTime}${t.endTime?' – '+t.endTime:''} · ${esc(t.category)}</div>
    </div>`;
  });
  ...
  if(isToday){
    ...
    if(upcoming.length){
      ...
      upcoming.sort((a,b)=>a.startTime.localeCompare(b.startTime)).forEach(t=>{
        html+=`<div class="cal-timeline-item"><span class="cal-timeline-time">${t.startTime}</span><span>${esc(t.title)}</span></div>`;
      });
      html+='</div>';
    }
    ...
    if(scheduled.length){
      ...
      scheduled.forEach(sl=>{
        if(sl.s>cursor)html+=`<div class="cal-timeline-item" style="background:var(--success-bg)"><span class="cal-timeline-time">${cursor} – ${sl.s}</span><span>Free</span></div>`;
        if(sl.e>cursor)cursor=sl.e;
      });
      if(cursor<'23:00')html+=`<div class="cal-timeline-item" style="background:var(--success-bg)"><span class="cal-timeline-time">${cursor} – 23:00</span><span>Free</span></div>`;
      html+='</div>';
    }
  }
