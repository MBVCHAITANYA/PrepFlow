...
function renderAnalytics(){
  ...
  const allCats=categories.filter(cat=>tasks.some(t=>t.category===cat));
  // Also include categories from existing tasks not in user's list
  const taskCats=[...new Set(tasks.map(t=>t.category).filter(Boolean))];
  const catsToShow=[...new Set([...allCats,...taskCats])];

  // Category grid
  const grid=document.getElementById('analyticsCategoryGrid');grid.innerHTML='';
  if(!catsToShow.length){
    grid.innerHTML='<div class="empty-state" style="grid-column:1/-1"><div class="empty-icon">📊</div><h3>No task data available yet</h3><p>Create tasks and complete them to see analytics.</p></div>';
  } else {
